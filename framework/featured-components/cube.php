<?php if ( get_theme_mod('playon_fe_enable') && is_front_page() ) : ?>
    <div id="cube" class="container-fluid featured-area container">
        ...
        <div class="featured-posts-container container">
            <?php if(get_theme_mod('playon_fe_cat1_enable')): ?>
            ...
            <?php endif; ?>
            <!-- Ctaegory 2 -->
        <?php if(get_theme_mod('playon_fe_cat2_enable')): ?>
    ...
    <?php endif; ?>
            <!-- Category 3 -->
    <?php if(get_theme_mod('playon_fe_cat3_enable')): ?>
        <div class="fg-wrapper1">
                <?php
                $count = 1;
                $args = array(
                    'post_type' => 'post',
                    'posts_per_page' => 3,
                    'cat'  => esc_html( get_theme_mod('playon_fe_cat3',0) ),
                    'ignore_sticky_posts' => 1,
                );
                $loop = new WP_Query( $args );
                while ( $loop->have_posts() ) :
                ...
                <?php wp_reset_postdata(); ?>
                <div class="view-all1">
                    <h3> <a href="<?php echo esc_url(get_category_link(get_theme_mod('playon_fe_cat3',0))); ?>">
                            <span class="title-font"><?php _e('View All','playon'); ?></span>
                        </a></h3>
                </div>
            </div>
